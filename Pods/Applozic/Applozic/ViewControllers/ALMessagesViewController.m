...
@interface ALMessagesViewController ()<UITableViewDataSource, UITableViewDelegate, ALMessagesDelegate, ALMQTTConversationDelegate, UITextFieldDelegate>
...
@property (nonatomic, strong) NSMutableArray * contactsMessageListArray;
@property (nonatomic, strong) NSMutableArray * filteredListArray;
...
-(IBAction)handleValueChanged:(UITextField*)textField
{
    _filteredListArray = [NSMutableArray new];
    
    NSString *searchText = _txtSearch.text;
    
    for (int i = 0; i < _contactsMessageListArray.count; i++)
     {
        ALMessage *messageObj = [_contactsMessageListArray objectAtIndex:i];
        
        ALContactDBService *contactDBService = [[ALContactDBService alloc] init];
        ALContact *alContact = [contactDBService loadContactByKey:@"userId" value: messageObj.to];
        
        NSString *message = alContact.displayName;
        
        if ([message rangeOfString:searchText options:NSCaseInsensitiveSearch].location != NSNotFound)
        {
            [_filteredListArray addObject:messageObj];
        }
     }
    
    if ([searchText isEqualToString:@""])
     {
        _filteredListArray = _contactsMessageListArray;
     }
...
-(IBAction)menuBtnAction:(UIButton*)button
{
    if (button.tag == 1001)
    {
        [self.navigationController pushViewController:_notfiVC animated:NO];
    }
    else if (button.tag == 1003)
    {
        [self.navigationController pushViewController:_homeVC animated:NO];
    }
    else if (button.tag == 1004)
    {
       [self.navigationController pushViewController:_favVC animated:NO];
    }
    else if (button.tag == 1005)
    {
        [self.navigationController pushViewController:_profileVC animated:NO];
    }
}
...
-(void)emptyConversationAlertLabel
{
    if(self.filteredListArray.count == 0)
    {
...
-(void)setUpTableView
{
    self.contactsMessageListArray = [NSMutableArray new];
    self.filteredListArray = [NSMutableArray new];
...
-(void)getMessagesArray:(NSMutableArray *)messagesArray
{
    ...
    self.contactsMessageListArray = messagesArray;
    self.filteredListArray = self.contactsMessageListArray;
    [self.mTableView reloadData];
...
//            [voipHandler handleAVMsg:msg andViewController:self];
        ...
        {
           index = [self.contactsMessageListArray indexOfObjectPassingTest:^BOOL(ALMessage *almessage, NSUInteger idx, BOOL *stop) {
            ...
            if (index != NSNotFound)
            {
                [self.contactsMessageListArray replaceObjectAtIndex:index withObject:msg];
            }
            else
            {
                //No cell founds
                    ...
                    if (msg.groupId)
                    {
                        ...
                        [channelService getChannelInformation:msg.groupId orClientChannelKey:nil withCompletion:^(ALChannel *alChannel) {
                            ...
                            BOOL channelFlag = [alChannel.parentKey isEqualToNumber:self.parentGroupKey];
                             ...
                             {
                                 [self.contactsMessageListArray insertObject:msg atIndex:0];
                                 self.filteredListArray = self.contactsMessageListArray;
                                 [self.mTableView reloadData];
                             }
                         }];
...
{
    int index = (int)[self.contactsMessageListArray indexOfObjectPassingTest:^BOOL(id element, NSUInteger idx, BOOL *stop) {
...
{
    return self.filteredListArray.count>0?[self.filteredListArray count]:0;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        contactCell.unreadCountLabel.layer.cornerRadius = 10;
    ...
    ALMessage *message = (ALMessage *)self.filteredListArray[indexPath.row];
...
{    
    ALMessage *message = self.filteredListArray[indexPath.row];
...
-(void)createDetailChatViewController:(NSString *)contactIds
{
    ...
    self.detailChatViewController.fromMessageList = YES;
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        ALMessage * alMessageobj = self.filteredListArray[indexPath.row];
        
        ALChannelService *channelService = [ALChannelService new];
        
        if([channelService isChannelLeft:[alMessageobj getGroupId]])
        {
            NSArray * filteredArray = [self.filteredListArray filteredArrayUsingPredicate:
        ...
        [ALMessageService deleteMessageThread:alMessageobj.contactIds orChannelKey:[alMessageobj getGroupId]
            ...
            if([alMessageobj getGroupId])
            {
                ...
                theFilteredArray = [self.filteredListArray filteredArrayUsingPredicate:
                                    [NSPredicate predicateWithFormat:@"groupId = %@",[alMessageobj getGroupId]]];
            }
            else
            {
...
-(void)subProcessDeleteMessageThread:(NSArray *)theFilteredArray
{
    ...
    [self.contactsMessageListArray removeObjectsInArray:theFilteredArray];
    [self emptyConversationAlertLabel];
    self.filteredListArray = self.contactsMessageListArray;
    [self.mTableView reloadData];
}
...
-(void)updateConversationTableNotification:(NSNotification *)notification
{
    ...
    NSArray * theFilteredArray = [self.contactsMessageListArray
    ...
    {
        [self.contactsMessageListArray removeObject:theLatestMessage];
        [self.contactsMessageListArray insertObject:theMessage atIndex:0];
       self.filteredListArray = self.contactsMessageListArray;
        [self.mTableView reloadData];
    }
}
